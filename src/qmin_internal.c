...
const struct qmin_hdr_tbl qmin_stx_tab[QMIN_STATIC_TABLE_N_ENTRIES] =
{
...
const struct qmin_huff_encode qmin_huff_encode_tables[257] =
{
    {     0x1ff8,    13},    //        (  0)
    {   0x7fffd8,    23},    //        (  1)
    {  0xfffffe2,    28},    //        (  2)
...
const struct qmin_huff_decode qmin_huff_decode_tables[256][16] =
{
    /* 0 */
    {
        { 4, 0x00, 0 },
...
{
    if (c & QMM_DUPLICATE)
        return QMM_DUPLICATE;
    if (c & QMM_INSERT)
        return QMM_INSERT;
    if (c & 0x20)
        return -1;
    if (c & QMM_STREAM_DONE)
        return QMM_STREAM_DONE;
    if (c & 0x08)
        return -1;
    if (c & QMM_DROP_CHKPOINT)
        return QMM_DROP_CHKPOINT;
    if (c & 0x02)
        return c;
    return -1;
}
...
{
    uint32_t B, M;
    const unsigned prefix_max = (1 << prefix_bits) - 1;
    ...
    *value = (*(*src)++ & prefix_max);
    ...
    if (M <= sizeof(*value) * 8)
        ...
        return DEC_ST_ERR;  /* Overflow */  /* XXX Should we treat it as
                                             * DEC_ST_NOBUF_DST instead?
                                             */
}
